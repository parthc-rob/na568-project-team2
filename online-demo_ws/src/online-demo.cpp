...
int main(int argc, char** argv)
{
	...
	int gpu_id;
	bool full_transform;
	nh.param<std::string>("net_def", net_def, "$(find deeplcd)/../devel/calc_model/deploy.prototxt");
	nh.param<std::string>("weights", weights, "$(find deeplcd)/../devel/calc_model/calc.caffemodel");
	nh.param<std::string>("pos_topic", pos_topic, "/kitti/transformStamped");
	nh.param<std::string>("im_topic", im_topic, "/kitti/camera_gray_left/image_rect");
	nh.param<int>("gpu", gpu_id, 0);
	nh.param<int>("min_loop_hyp_cnt", min_loop_hyp_cnt, 3);
	nh.param<double>("base_thresh", thresh, 0.9);
	nh.param<bool>("full_transform", full_transform, 1);
	...
	marker_pub = nh.advertise<visualization_msgs::Marker>("visualization_marker", 10);
    	...
    	loop_lines.header.frame_id = line_strip.header.frame_id  = "/world";
